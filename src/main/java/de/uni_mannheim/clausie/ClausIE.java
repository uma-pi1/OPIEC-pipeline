...
public class ClausIE {
    ...
    private ObjectArrayList<Clause> clauses = new ObjectArrayList<>();
    private PropositionGenerator propositionGenerator = new DefaultPropositionGenerator(this);
    ...
    public ClausIE(SemanticGraph sg) {
        this(new Options());
        this.semanticGraph = sg;
        this.detectClauses();
        this.generatePropositions(this.semanticGraph);
    }
    ...
    public void generatePropositions(final SemanticGraph sGraph) {
        ...
        final ObjectArrayList<ObjectArrayList<Constituent>> constituents = new ObjectArrayList<>();

        // Which of the constituents are required?
        final ObjectArrayList<Status> flags = new ObjectArrayList<>();
        ...
        final ObjectArrayList<BooleanArrayList> includeConstituents = new ObjectArrayList<>();
        ...
        for (Clause clause : clauses) {
            ...
            for (int i = 0; i < clause.getConstituents().size(); i++) {
                ...
                if (!(xcomp && clause.getSubject() == i)
                    ...
                    alternatives = ProcessConjunctions.processCC(constituent);
                } else if (!(xcomp && clause.getSubject() == i) && clause.getXcompsInds().contains(i)) {
                    alternatives = new ObjectArrayList<>();
                    ...
                    for (Clause cl: xclausIE.getClauses()){
                        for (Proposition p : cl.getPropositions()) {
                            ...
                            for (int j = 0; j < p.getPhrases().size(); j++) {
                                ...
                                //XXX
                                if(p.getPhrases().get(j).isProcessedConjunction()){
                                    phrase.setProcessedConjunction(p.getPhrases().get(j).isProcessedConjunction());
                                    phrase.setConjWord(p.getPhrases().get(j).getConjWord());
                                }
                            }
                    ...
                    alternatives = new ObjectArrayList<>(1);
