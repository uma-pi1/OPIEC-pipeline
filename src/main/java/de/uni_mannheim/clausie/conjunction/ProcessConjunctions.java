...
public class ProcessConjunctions {
    ...
    public static ObjectArrayList<Constituent> processCC(Constituent constituent) {
        return generateConstituents((IndexedConstituent) constituent);
    }

    /** Generates a set of constituents from a CC for a given constituent */
    private static ObjectArrayList<Constituent> generateConstituents(IndexedConstituent constituent) {
        IndexedConstituent copy = constituent.clone();
        copy.setSemanticGraph( copy.createReducedSemanticGraph() );
        ObjectArrayList<Constituent> result = new ObjectArrayList<>();
        result.add(copy);
    ...
    private static void generateConstituents(SemanticGraph semanticGraph, IndexedConstituent constituent, IndexedWord root,
        ...
        for (SemanticGraphEdge edge : outedges) {
            ...
            if (DpUtils.isAnyConj(edge) && processCC) {
                ...
                if (conjunct != null) {
                    for(SemanticGraphEdge c : conjunct) {
                        if (c.getDependent().lemma().equals("&") &&
                                nextToVerb(root, edge.getDependent(), semanticGraph.getOutEdgesSorted(root))) {
                            cont = true;
                            break;
                        }
                    }
                ...
                //XXX
                newConstituent.setProcessedConjunction(true);
                newConstituent.setConjWord(newRoot);
    ...
    public static List<IndexedWord> getIndexedWordsConj(SemanticGraph semanticGraph, IndexedWord root,
        ...
        for (SemanticGraphEdge edge : outedges) {
            if (edge.getRelation().equals(rel)) {
                ...
                boolean notProcess = !option.processCcAllVerbs && outed.isEmpty()
                        && shareAll(outedges, root, edge.getDependent(), semanticGraph);
                    ...
                    if (option.processCcAllVerbs || !notProcess) {
                        ...
                        for (SemanticGraphEdge e : conjunct) {
                            if (e.getDependent().index() > edge.getDependent().index())
                                continue;
                            if (nextToVerb(root, edge.getDependent(), semanticGraph.getOutEdgesSorted(root))) {
                                toRemove.add(e);
                                break;
                            }
                        }
        ...
        if (ccs.size() > 1) rewriteGraph(semanticGraph, ccs);
    ...
    private static boolean nextToVerb(IndexedWord firstVerb, IndexedWord secondVerb, List<SemanticGraphEdge> sortedOutEdges){
        ...
        if (conjIndex > 0){
            ...
            return leftConjEdge.getRelation().getShortName().equals("cc");
        } else {
            return false;
        }
    }
