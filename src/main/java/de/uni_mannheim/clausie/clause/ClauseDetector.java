...
import de.uni_mannheim.constant.EXTRACTION_TYPE;
...
public class ClauseDetector {
    ...
    static {
        HashSet<GrammaticalRelation> temp = new HashSet<>();
    ...
    public static void detectClauses(ClausIE clausIE) {
        ...
        for (SemanticGraphEdge edge : clausIE.getSemanticGraph().edgeIterable()) {
            ...
            if (DpUtils.isAnySubj(edge)) {
                ...
                IndexedWord possVerb = clausIE.getSemanticGraph().getChildWithReln(subject, EnglishGrammaticalRelations.POSSESSIVE_MODIFIER);
                if (possVerb == null) {
                    addPossessiveClause(clausIE, subject, object);
                    roots.add(null);
                } else if (possVerb != null && possVerb.ner().equals(NE_TYPE.NO_NER)) {
                    addPossessiveClause(clausIE, subject, object);
                    roots.add(null);
                }
     ...
     */
    public static void addNsubjClause(ClausIE clausIE, List<IndexedWord> roots,
        ...
        // flags for synthetic clauses
        boolean vmodFlag = false;
        boolean vmodCopFlag = false;
...
//------------------------Set verb or complement, and subject.-------------------------------------------------
            ...
            if (cop != null) {
                ...
                constRoot = new IndexedConstituent(semanticGraph, root, emptySet(), exclude, Type.COMPLEMENT);
                clause.constituents.add(constRoot);

                clause.verb = clause.constituents.size();
                if (!partmod) {
                    clause.constituents.add(new IndexedConstituent(semanticGraph, cop.getDependent(), include, emptySet(), Type.VERB));
                } else {
                    ...
                    bePhrase.setHeadWord(clauseRoot);
                    vmodCopFlag = true;
                    clause.constituents.add(new PhraseConstituent(bePhrase, Type.VERB));
                }

            } else {
                clause.verb = clause.constituents.size();
                if (!partmod) {
                    constRoot = new IndexedConstituent(semanticGraph, root, emptySet(), exclude, Type.VERB);
                } else {
                    // Make a phrase consisted of be + clauseRoot
                    Phrase bePhrase = new Phrase();
                    IndexedWord beWord = new IndexedWord();
                    ...
                    bePhrase.setHeadWord(clauseRoot);
                    vmodFlag = true;
                    constRoot = new PhraseConstituent(bePhrase, Type.VERB);
                }
            ...
            if (subject.tag().charAt(0) == 'W' && rcmod != null) {
                ...
                if (sub != null)
                    ...
                    clause.constituents.add(new IndexedConstituent(semanticGraph, subject, Type.SUBJECT));
            ...
            for (SemanticGraphEdge outgoingEdge : outgoingEdges) {
                ...
                //Indirect Object
                    ...
                    // "regular case"    
                        ...
                        clause.constituents.add(new IndexedConstituent(semanticGraph, dependent, Type.IOBJ));
                //Direct Object
                    ...
                    if (dependent.tag().charAt(0) == 'W' && rcmod != null) {
                        ...
                        clause.constituents.add(new IndexedConstituent(semanticGraph, dependent, Type.DOBJ));
                //CCOMPS
                } else if (DpUtils.isCcomp(outgoingEdge)) {
                    clause.ccomps.add(clause.constituents.size());
                    clause.constituents.add(new IndexedConstituent(semanticGraph, dependent, Type.CCOMP));
                ...
                // are managed within the context of its parent clause)
                    ...
                    ObjectArrayList<Clause> xcompclauses = new ObjectArrayList<>();
                    ...
                    clause.constituents.add(new XcompConstituent(semanticGraph, dependent, Type.XCOMP,
                                                                 xcompclauses));
                 //Adjective complement
                } else if (DpUtils.isAcomp(outgoingEdge)) { 
                    clause.acomps.add(clause.constituents.size());
                    clause.constituents.add(new IndexedConstituent(semanticGraph, dependent, Type.ACOMP));
                 //Various Adverbials
					...
					clause.constituents.add(new IndexedConstituent(semanticGraph, dependent, Type.ADVERBIAL));
				//Advmod
                 ...
                 // We generate a new clause out of the relative clause   
                	...
                	processRel(semanticGraph, dependent, rcmod, clause);
                	rcmod = null;
               ...
               // else if (DpUtils.isMark(outgoingEdge) || DpUtils.isComplm(outgoingEdge)) {
                    ...
                    clause.setExtractionType(Clause.Type.EXISTENTIAL.toString(), false);
                }
 ...
 //------------------------To process relative clauses with implicit (zero) relative pronoun-------------------------
            if (rcmod != null) { //"I saw the house I grew up in", "I saw
                ...
                if (candidate != null) {
                    ...
                    newSemanticGraph.addEdge(candidate.getRoot(), rcmod.getGovernor(),
...
//------------------------------------------------------------------------------------------------------------------          
            ...
            if (!partmod) {
                ...
                clause.setExtractionType(clause.getType().toString(), false);
            } else {
                clause.setType(Clause.Type.SVA);
                clause.setExtractionType(Clause.Type.SVA.toString(), false);
            }

            // Set extraction types to be different than clause types if clauses are synthetic
            if (vmodFlag) {
                clause.setExtractionType(EXTRACTION_TYPE.IMPL_VMOD, true);
            }
            if (vmodCopFlag) {
                clause.setExtractionType(EXTRACTION_TYPE.IMPL_VMOD_COP, true);
            }
     ...
     */
    private static void processRel(SemanticGraph semanticGraph, IndexedWord dependent, SemanticGraphEdge rcmod, Clause clause) {
    ...
    */
    private static Constituent createPossConstituent(SemanticGraph semanticGraph,
        ...
        return new IndexedConstituent(newSemanticGraph, constGovernor, emptySet(), exclude, type);
    }
    ...
    private static IndexedConstituent createRelConstituent(SemanticGraph semanticGraph, IndexedWord root, Type type) {
        ...
        if (rccop != null) {
            ...
            return new IndexedConstituent(semanticGraph, root, emptySet(), excludercmod, type);
        } else
    ...
    private static void addApposClause(ClausIE clausIE, IndexedWord subject, IndexedWord object) {
        ...
        Type argumentConstType = Type.COMPLEMENT;
        ...
        apposPhrase.setHeadWord(appositionVerb);
        
        PhraseConstituent verbConstit = new PhraseConstituent(apposPhrase, Type.VERB);
        ...
        if ((subject.ner().equals(NE_TYPE.ORGANIZATION) || subject.ner().equals(NE_TYPE.LOCATION))
                && object.ner().equals(NE_TYPE.LOCATION)){
            ...
            verbConstit = new PhraseConstituent(apposPhrase, Type.VERB);
            argumentConstType = Type.ADVERBIAL;
            clause.adverbials.add(2);
            clause.complement = -1;
        } else if (!argsNERCheckForBeRel(subject, object)) {
            return;
        }

        clause.constituents.add(new IndexedConstituent(clausIE.getSemanticGraph(), subject, Type.SUBJECT));
        ...
        clause.setExtractionType(EXTRACTION_TYPE.IMPL_APPOS_CLAUSE, true);
    ...
    */
    private static void addPossessiveClause(ClausIE clausIE, IndexedWord subject, IndexedWord object) {
        ...
        if (rcmod != null) {
            ...
            clause.constituents.add(new IndexedConstituent(newSemanticGraph, subject, emptySet(), excludesub, Type.SUBJECT));
        }
        ...
        possPhrase.setHeadWord(possessiveVerb);
        
        clause.constituents.add(new PhraseConstituent(possPhrase, Type.VERB));
        clause.constituents.add(new IndexedConstituent(newSemanticGraph, object, emptySet(), excludeobj, Type.DOBJ));
        clause.setType(Clause.Type.SVO);
        clause.setExtractionType(EXTRACTION_TYPE.IMPL_POSS_CLAUSE, true);
        clausIE.getClauses().add(clause);
    }
    ...
    */
    ...
    private static void addPartmodClause(ClausIE clausIE, IndexedWord subject, IndexedWord verb, List<IndexedWord> roots) {
        addNsubjClause(clausIE, roots, clausIE.getClauses(), subject, verb, true);
    }
    ...
    private static void addParataxisClause(ClausIE clausIE, IndexedWord root, IndexedWord parroot, List<IndexedWord> roots) {
        ...
        if (subject != null) {
            Constituent subjectConst = new IndexedConstituent(clausIE.getSemanticGraph(), subject.getDependent(), Type.SUBJECT);
            ...
            clause.setType(Clause.Type.SVO);
            clause.setExtractionType(Clause.Type.SVO.toString(), false);
            clausIE.getClauses().add(clause);
            roots.add(null);
        }
    }
