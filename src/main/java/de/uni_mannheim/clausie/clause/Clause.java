...
public class Clause {
    ...
    private BooleanArrayList include;

    /** List of propositions generated from the clause **/
    private ObjectArrayList<Proposition> propositions;

    /** In most cases, same as clause type. Differs for synthetic clauses, so that we can differentiate
     * the extractions which are found explicitly in the sentence from the ones that are not **/
    ...
    Clause() {
        this.constituents = new ObjectArrayList<>();
    ...
    public Clause clone() {
        ...
        clause.constituents = new ObjectArrayList<>(constituents);
        clause.type = type;
    ...
    void detectType(Options options) {
        ...
        IndexedWord root = constituents.get(verb).getRoot();
        ...
        if (hasDirectObject || hasIndirectObject) {
            ...
            // Q11: Conservative?
            if (options.conservativeSVOA) type = Type.SVOA;
            else {
                type = Type.SVO;
            }
        } else {
    ...
    private boolean hasCandidateAdverbial() {
        ...
        return constituents.get(adverbials.getInt(adverbials.size() - 1)).getRoot().index() >
                constituents.get(verb).getRoot().index();
    }
    ...
    public String toString(Options options) {
        ...
        for (int index = 0; index < constituents.size(); index++) {
            ...
            //s.append(constituent.rootString());
            if (constituent instanceof IndexedConstituent) {
                s.append(constituent.getRoot().word());
                s.append("@");
                s.append(constituent.getRoot().index());
            } else {
                s.append("\"");
            }
        }
    ...
    public Status getConstituentStatus(int index, Options options) {
        ...
        for (int i : adverbials) {
            ...
            int adv = constituents.get(i).getRoot().index();
            if (constituents.get(verb) instanceof IndexedConstituent && 
                    adv < constituents.get(verb).getRoot().index() && !relativeAdverbial) {
                if (i == index) {
    ...
    private boolean isIgnoredAdverbial(int index, Options options) {
        ...
        return options.dictAdverbsIgnore.contains(s) || (options.processCcNonVerbs && options.dictAdverbsConj.contains(s));
    }
    ...
    public void setExtractionType(String extractionType, boolean override) {
        if (override) {
            this.extractionType = extractionType;
        } else {
            if (this.extractionType.equals("")) {
                this.extractionType = extractionType;
            } else {
                return;
            }
