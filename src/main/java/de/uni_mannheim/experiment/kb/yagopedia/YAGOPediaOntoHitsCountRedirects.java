...
public class YAGOPediaOntoHitsCountRedirects {
    ...
    public static void main(String args[]) throws IOException {
        System.err.println("Loading map of redirects ... ");
        RedirectLinksMap linkMap = new RedirectLinksMap("/redirects/redirects.dict");
        System.err.println("Map of redirects loaded!");
        ...
        for (int i = 0; i < listOfFiles.length; i++) {
            ...
            while (dataFileReader.hasNext()) {
                ...
                // Redirects (canonical links)
                if (linkMap.getRedirectMap().containsKey(subjectLink)) {
                    subjCanonicalLink = KBHitsCounting.makeUppercasedPageTitle(linkMap.getRedirectMap().get(subjectLink));
                    //subjCanonicalLink = String.join("_", linkMap.getRedirectMap().get(subjectLink).split(" "));
                } else {
                    subjCanonicalLink = subjectLink;
                }
                if (linkMap.getRedirectMap().containsKey(objectLink)) {
                    //objCanonicalLink = String.join("_", linkMap.getRedirectMap().get(objectLink).split(" "));
                    objCanonicalLink = KBHitsCounting.makeUppercasedPageTitle(linkMap.getRedirectMap().get(objectLink));
                } else {
        ...
        System.out.println("\n\nTriples count: " + tripleCount); // String.format("%.1f", (double) 589/ (double) 856))
