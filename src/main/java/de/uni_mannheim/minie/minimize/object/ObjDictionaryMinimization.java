...
public class ObjDictionaryMinimization {
     ...
     * @param collocations: dictionary of multi-word expressions (frequent objects)
     */
    public static void minimizeObject(AnnotatedPhrase obj, SemanticGraph sg, ObjectOpenHashSet<String> collocations){
        ...
        if (collocations.contains(WordCollectionUtils.toLemmaString(obj.getWordList()).toLowerCase())){
            return;
        }
        
        // Minimization object
        Minimization minimization = new Minimization(obj, sg, collocations);
